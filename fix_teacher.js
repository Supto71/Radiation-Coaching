...
let content = fs.readFileSync('frontend/src/pages/TeacherDashboard.jsx', 'utf8');
...
const old_add_question =   const addQuestion = async (e) => {
    e.preventDefault();
...
const new_add_question =   const saveAllQuestions = async (e) => {
    e.preventDefault();
    if (!selectedExamId) return alert('??? ??????? ???????? ????!');
    ...
    for (let i = 0; i < cleanedList.length; i++) {
      ...
      if (cleanedList[i].options.some(opt => opt === '')) return alert(\\ ?? ???????? ?????? ???? ???? ????!\);
    }
...
// 4. The form UI
const old_form =                   <form onSubmit={addQuestion} className="space-y-6 bg-gray-50 p-6 md:p-7 rounded-2xl border border-gray-200">
...
const new_form =                   <form onSubmit={saveAllQuestions} className="space-y-6 bg-gray-50 p-6 md:p-7 rounded-2xl border border-gray-200">
...
fs.writeFileSync('frontend/src/pages/TeacherDashboard.jsx', content, 'utf8');
console.log('Done');
